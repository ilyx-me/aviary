...
in {
  ...
  config = {
    ...
    boot.initrd.network.ssh.authorizedKeys = lib.mkForce config.users.users."999".openssh.authorizedKeys.keys;

    users.users = {

      root.openssh.authorizedKeys.keys = config.users.users."999".openssh.authorizedKeys.keys;
      "999".openssh.authorizedKeys.keys = [ admin-ibis admin-chicken ];
